...
<template>
  <div v-if="show && transaction" class="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4">
    <div class="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-2xl overflow-hidden my-6 transform transition-all">
      ...
      <div class="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 px-6 py-4 text-white flex items-center justify-between">
        <div>
          <span class="text-[10px] uppercase font-bold text-indigo-300 tracking-wider">Transaction Detail Modal</span>
          <h3 class="text-base font-bold flex items-center gap-2">
            <span>🧾</span> Transaction #{{ transaction.txn_id || transaction.order_id }} 
            <span class="text-xs bg-emerald-500 text-white px-2 py-0.5 rounded-full font-bold">✅ SUCCESS</span>
          </h3>
        </div>
      ...
      <div class="p-6 space-y-5 text-xs text-slate-700 max-h-[75vh] overflow-y-auto">
        ...
        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-1.5">
          <h4 class="font-bold text-slate-900 uppercase text-[11px] text-indigo-700 flex items-center gap-1">
            <span>👤</span> CUSTOMER INFO
          </h4>
          <div class="grid grid-cols-2 gap-2 text-xs">
            <div><span class="text-slate-500">Name:</span> <strong class="text-slate-900">{{ transaction.student?.name }}</strong></div>
            <div><span class="text-slate-500">ID:</span> <strong class="font-mono text-slate-800">{{ transaction.student?.id }}</strong></div>
            <div><span class="text-slate-500">Email:</span> <span class="text-slate-700">{{ transaction.student?.email }}</span></div>
            <div><span class="text-slate-500">Phone:</span> <span class="font-mono text-slate-700">{{ transaction.student?.phone }}</span></div>
          </div>
        </div>

        <!-- Purchase Details -->
        <div class="bg-indigo-50/50 p-4 rounded-2xl border border-indigo-100 space-y-1.5">
          <h4 class="font-bold text-indigo-900 uppercase text-[11px] flex items-center gap-1">
            <span>📚</span> PURCHASE DETAILS
          </h4>
          <div class="grid grid-cols-2 gap-2 text-xs">
            <div><span class="text-slate-500">Course:</span> <strong class="text-indigo-900">{{ transaction.course }}</strong></div>
            <div><span class="text-slate-500">Teacher:</span> <strong class="text-slate-800">{{ transaction.teacher }}</strong></div>
            <div class="col-span-2"><span class="text-slate-500">Semester:</span> <span class="text-slate-700">{{ transaction.semester }}</span></div>
          </div>
        </div>

        <!-- Payment Breakdown -->
        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-1.5 font-mono">
          <h4 class="font-bold text-slate-900 uppercase text-[11px] font-sans text-emerald-700 flex items-center gap-1">
          ...
          <div class="flex justify-between text-sm font-bold text-slate-900 pt-1 border-t border-slate-200">
            ...
            <span class="text-emerald-600">${{ (transaction.amount || 45).toFixed(2) }} USD</span>
          </div>
        </div>

        <!-- ABA PayWay Details -->
        <div class="bg-slate-900 text-white p-4 rounded-2xl space-y-2 text-xs font-mono">
        ...
        <!-- Timeline -->
        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200/80 space-y-1 text-xs">
          <h4 class="font-bold text-slate-800 uppercase text-[11px] flex items-center gap-1">
        ...
        <!-- Revenue Split -->
        <div class="bg-indigo-50/70 p-4 rounded-2xl border border-indigo-100 flex items-center justify-between font-mono text-xs">
          <div>
            <span class="text-slate-500 block text-[10px]">Teacher Share (70%)</span>
            <span class="font-bold text-indigo-900">${{ (transaction.teacher_share_amount || 31.50).toFixed(2) }} → Pending payout</span>
          </div>
          <div class="text-right">
            <span class="text-slate-400 block text-[10px]">Platform Share (30%)</span>
            <span class="font-bold text-emerald-700">${{ (transaction.platform_share_amount || 13.50).toFixed(2) }} → Received</span>
          </div>
        </div>
