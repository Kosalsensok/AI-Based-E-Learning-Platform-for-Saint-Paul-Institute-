...
const openEdit = (row: any) => {
  ...
  form.code = row.code || ''
  form.faculty_id = row.faculty_id
...
const submit = () => {
  if (editMode.value && selected.value) {
    form.put(route('admin.departments.update', selected.value.id), {
      preserveScroll: true,
      onSuccess: () => {
        dialog.value = false
        toast.add({ severity: 'success', summary: 'Success', detail: 'Department updated' })
      },
    })
  } else {
    form.post(route('admin.departments.store'), {
      ...
      onSuccess: () => {
        dialog.value = false
        toast.add({ severity: 'success', summary: 'Success', detail: 'Department created' })
      },
    })
  }
}
  ...
  <AdminLayout title="Departments">
    <div class="p-6 space-y-4">
      ...
      <div class="flex items-center justify-between">
        ...
        <Button label="+ Create Department" icon="pi pi-plus" @click="openCreate" />
      </div>

      <DataTable :value="props.departments" :rows="10" stripedRows paginator>
        ...
        <Column header="Faculty">
          <template #body="{ data }">
            <span class="font-semibold">{{ data.faculty?.name }}</span>
          </template>
        </Column>
      ...
      <Dialog v-model:visible="dialog" modal :header="editMode ? 'Edit Department' : 'Create Department'" :style="{ width: '30rem' }">
        <div class="space-y-4">
          <div>
